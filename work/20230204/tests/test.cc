...
class IconDisplayer {
    public :
        IconDisplayer(HWND hwnd) : hwnd_{hwnd} {
            ...
            data_.hWnd = hwnd_;
            data_.uID = 1;    
            HICON hIcon = (HICON)LoadImage(NULL, fname, IMAGE_ICON, 16, 16, LR_LOADFROMFILE | LR_SHARED);
...
LRESULT _stdcall CallbackFunc(int code, WPARAM w_par, LPARAM l_par) {
    ...
    if (w_par != WM_KEYDOWN or code != HC_ACTION) return CallNextHookEx(NULL, code, w_par, l_par);    
